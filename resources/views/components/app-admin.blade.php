...
<body class="font-sans antialiased bg-gradient-to-br from-gray-50 via-red-50 to-yellow-50 min-h-screen">
    <div class="min-h-screen flex" x-data="{ sidebarCollapsed: false, mobileSidebarOpen: false }">
        ...
        <div class="fixed inset-y-0 left-0 z-50 transition-all duration-300 ease-in-out" 
            ...
            <div class="h-full bg-gradient-to-br from-white/95 to-slate-50/95 backdrop-blur-[20px] border-r border-white/20 shadow-[0_8px_32px_rgba(0,0,0,0.1)] relative overflow-hidden">
                <!-- Header -->
                ...
                <button @click="sidebarCollapsed = !sidebarCollapsed" 
                    ...
                    <svg class="w-3.5 h-3.5 text-white transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
                    </svg>
                </button>
                
                <!-- Navigation Container -->
                <div class="flex flex-col h-[calc(100vh-120px)] px-4 mt-8 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                    ...
                    <div class="mt-auto pt-4 border-t border-[#8F181A]/10">
                        ...
                        <div class="space-y-1">
                            ...
                            <a href="{{ route('dashboard') }}" class="flex items-center justify-center px-3 py-2 text-sm font-medium text-white bg-gradient-to-r from-[#8F181A] via-[#B91C1C] to-[#EDBC19] rounded-md hover:shadow-[0_4px_16px_rgba(143,24,26,0.3)] transition-all duration-300 hover:-translate-y-0.5 gap-1.5 shadow-[0_2px_8px_rgba(143,24,26,0.2)]" x-show="!sidebarCollapsed" x-transition>
                            ...
                            x-show="sidebarCollapsed" x-transition>
                                ...
                                <div class="absolute left-16 bg-gray-900 text-white px-2 py-1 rounded-md text-xs whitespace-nowrap z-50 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300">
                                    Back to Main Dashboard
                                </div>
                            </a>
                            ...
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit" class="flex items-center justify-center w-full px-3 py-2 text-sm font-medium text-white bg-gradient-to-r from-[#8F181A] via-[#B91C1C] to-[#EDBC19] rounded-md hover:shadow-[0_4px_16px_rgba(143,24,26,0.3)] transition-all duration-300 hover:-translate-y-0.5 gap-1.5 shadow-[0_2px_8px_rgba(143,24,26,0.2)]">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="flex-1 flex flex-col transition-all duration-300 ease-in-out" 
            ...
            <!-- Page Content -->
            <main class="flex-1 overflow-y-auto bg-transparent [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                <div class="p-8">
                    {{ $slot }}
                </div>
            </main>
